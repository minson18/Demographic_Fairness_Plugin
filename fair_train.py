...
def run_fair_training(fairness_lambda=0.1, dataset="ml-1m", maxlen=100, batch_size=128, epochs=20):
    ...
    sys.argv.append(f"--save_dir={save_dir}")
    
    # Log experiment settings
    print(f"Starting fairness-aware training with lambda={fairness_lambda}")
    print(f"Dataset: {dataset}, Max length: {maxlen}, Batch size: {batch_size}, Epochs: {epochs}")
...
def compare_fairness_lambdas(lambdas=[0.0, 0.1, 0.5, 1.0], dataset="ml-1m", epochs=20):
    ...
    print("\nResults Comparison:")
    print(f"{'Lambda':10} {'NDCG@20':10} {'Hit@20':10} {'Gender DP':10} {'Age DP':10} {'Occ DP':10}")
    print(f"{'-'*70}")
    
    for lambda_val, metrics in results.items():
        print(f"{lambda_val:<10.2f} {metrics['ndcg@20']:<10.4f} {metrics['hit@20']:<10.4f} "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train recommendation models with controllable fairness")
    parser.add_argument("--lambdas", type=float, nargs="+", default=[0.0, 0.1, 0.5, 1.0],
                        help="List of fairness lambda values to compare")
    parser.add_argument("--dataset", type=str, default="ml-1m", help="Dataset to use")
    parser.add_argument("--epochs", type=int, default=20, help="Number of epochs for each run")
