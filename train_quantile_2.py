...
def binary_quantile_loss(pos_logits, neg_logits, mask, q=0.8, positive_weight=0.7):
    ...
    total_loss = positive_weight * pos_loss.sum() + (1 - positive_weight) * neg_loss.sum()
...
def train_one_epoch(model, dataloader, optimizer, device, alpha=0.05, beta=0.2):
    ...
    for batch in tqdm(dataloader, desc="Train", leave=False):
        ...
        quantile_loss = binary_quantile_loss(pos_logits, neg_logits, mask, q=0.8, positive_weight=0.7)
        quantile_loss_rank = ranking_quantile_loss(pos_logits, neg_logits, mask, q=0.8)
        loss = BCEloss * (1-alpha-beta) + quantile_loss * alpha + quantile_loss_rank * beta
        loss.backward()
        optimizer.step()
...
def train():
    ...
    for epoch in range(1, args.num_epochs + 1):
        loss = train_one_epoch(model, dataloader, optimizer, args.device, args.alpha, args.beta)
...
#     pos_error = y_pos - torch.sigmoid(pos_logits)
#     neg_error = y_neg - torch.sigmoid(neg_logits)
...
#         loss = binary_quantile_loss(pos_logits, neg_logits, mask, q=0.8, positive_weight=0.7)
#         loss.backward()
#         optimizer.step()
#         total_loss += loss.item()
#     return total_loss / len(dataloader)
...
#     best_model_path = os.path.join(save_dir, "best_model_quantile.pth")
