...
class Evaluator:
    def __init__(
        ...
        self.candidate_indices = [self.itemid2idx[item] for item in self.candidate_items]
        self.candidate_item_features = torch.tensor(
            self.item_features[self.candidate_indices], dtype=torch.float32, device=self.device
        )
        # Cache user features as tensors for fast access
        self.user_features_tensor = torch.tensor(self.user_features, dtype=torch.float32, device=self.device)
    ...
    def _build_sequence_tensors(self, train_seq: List[int], user: int) -> tuple:
        ...
        mapped_train_seq = [self.itemid2idx[i] for i in train_seq if i in self.itemid2idx]
        if len(mapped_train_seq) > maxlen - 1:
            mapped_train_seq = mapped_train_seq[-(maxlen - 1):]
        seq = np.zeros([maxlen], dtype=np.int32)
        seq[-(len(mapped_train_seq)+1):-1] = mapped_train_seq if len(mapped_train_seq) > 0 else []
        seq_tensor = torch.tensor(seq, dtype=torch.long, device=self.device).unsqueeze(0)
        seq_feat = torch.zeros((1, maxlen, self.item_features.shape[1]), dtype=torch.float32, device=self.device)
        if len(mapped_train_seq) > 0:
            seq_feat[0, -(len(mapped_train_seq)+1):-1] = torch.tensor(self.item_features[mapped_train_seq], dtype=torch.float32, device=self.device)
        seqcxt = torch.zeros((1, maxlen, self.model.cxt_size), dtype=torch.float32, device=self.device)
        for idx, item in enumerate(seq_tensor.cpu().numpy()[0][-(len(mapped_train_seq)+1):-1]):
            if item != 0:
                orig_item = train_seq[idx] if idx < len(train_seq) else 0
                seqcxt[0, -(len(mapped_train_seq)+1)+idx, :] = torch.tensor(
                    self.cxtdict.get((user, orig_item), np.zeros(self.model.cxt_size)), dtype=torch.float32, device=self.device
                )
    ...
    def _build_candidate_context(self, user: int) -> torch.Tensor:
        ...
        """
        cxts = [self.cxtdict.get((user, item), np.zeros(self.model.cxt_size)) for item in self.candidate_items]
        cxts = np.array(cxts, dtype=np.float32)  # Convert list of arrays to a single numpy array
    ...
    def get_top_k(
        ...
        pos[:, -1] = torch.tensor(self.candidate_indices, dtype=torch.long, device=self.device)
        ...
        with torch.no_grad():
            pos_logits, _ = self.model(
                user_feat_batch, seq_batch, seq_feat_batch, seqcxt_batch, pos, pos_feat, poscxt, neg, neg_feat, negcxt
            )
    ...
    def get_top_k_batch(
        ...
        if swap_gender is not None:
            user_feats[:, 0] = torch.tensor(swap_gender, dtype=user_feats.dtype, device=self.device)
        if swap_age is not None:
            user_feats[:, 1] = torch.tensor(swap_age, dtype=user_feats.dtype, device=self.device)

        # Sequence/history tensors
        seqs = np.zeros((batch_size, maxlen), dtype=np.int32)
        seq_feats = np.zeros((batch_size, maxlen, self.item_features.shape[1]), dtype=np.float32)
        ...
        for i, (train_seq, user) in enumerate(zip(train_seqs, users)):
            mapped_train_seq = [self.itemid2idx[x] for x in train_seq if x in self.itemid2idx]
            if len(mapped_train_seq) > maxlen - 1:
                mapped_train_seq = mapped_train_seq[-(maxlen - 1):]
            seqs[i, -(len(mapped_train_seq)+1):-1] = mapped_train_seq if len(mapped_train_seq) > 0 else []
            if len(mapped_train_seq) > 0:
                seq_feats[i, -(len(mapped_train_seq)+1):-1, :] = self.item_features[mapped_train_seq]
            for idx, item in enumerate(seqs[i, -(len(mapped_train_seq)+1):-1]):
                if item != 0:
                    orig_item = train_seq[idx] if idx < len(train_seq) else 0
                    seqcxts[i, -(len(mapped_train_seq)+1)+idx, :] = self.cxtdict.get((user, orig_item), np.zeros(self.model.cxt_size))
        ...
        for chunk_start in range(0, num_candidates, candidate_chunk_size):
            ...
            pos_feat = seq_feats.unsqueeze(1).repeat(1, chunk_size, 1, 1).reshape(-1, maxlen, self.item_features.shape[1])
            poscxt = seqcxts.unsqueeze(1).repeat(1, chunk_size, 1, 1).reshape(-1, maxlen, self.model.cxt_size)
            candidate_indices_tensor = torch.tensor(chunk_indices, dtype=torch.long, device=self.device)
            for i in range(batch_size):
                pos[i*chunk_size:(i+1)*chunk_size, -1] = candidate_indices_tensor
                pos_feat[i*chunk_size:(i+1)*chunk_size, -1, :] = chunk_item_features
                poscxt[i*chunk_size:(i+1)*chunk_size, -1, :] = candidate_contexts[i]

            user_feat_batch = user_feats.unsqueeze(1).repeat(1, chunk_size, 1).reshape(-1, user_feats.shape[1])
            seq_batch = seqs.unsqueeze(1).repeat(1, chunk_size, 1).reshape(-1, maxlen)
            seq_feat_batch = seq_feats.unsqueeze(1).repeat(1, chunk_size, 1, 1).reshape(-1, maxlen, self.item_features.shape[1])
            seqcxt_batch = seqcxts.unsqueeze(1).repeat(1, chunk_size, 1, 1).reshape(-1, maxlen, self.model.cxt_size)
            neg = pos
            neg_feat = pos_feat
            neg_cxt = poscxt
    ...
    def sample_user_subset(self, user_eval: dict, percent: float = 0.1, seed: int = 42) -> dict:
        """
